...
Pawn::Pawn(PlayerColor color, int id) :
    ...
    // Isso não é o tamanho de desenho (consulte paintEvent())
...
unsigned int Pawn::getIndex() {
    // O índice é extraído usando o id do peão, já que os ids são sequenciais
...
void Pawn::changePosition(int rel) {
    if(rel > DEST || rel < HOME)
        ValueError::raise_new(QString("Pawn::goto(int) : Movimento inválido! rel == ").arg(rel));
...
void Pawn::moveForward(int steps) {
    if(this->mPos + steps > DEST || steps < 1 || steps > 6)
        ValueError::raise_new(QString("Pawn::moveForward(int) : Movimento inválido! steps == ")
                                  .arg(steps));
